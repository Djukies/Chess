...
void UCIBridge::sendToEngine(const std::string& command) {
...
void UCIBridge::handleUCI() {
    // Send engine identification
    std::cout << "id name DjukeBot 1.0" << std::endl;
...
void UCIBridge::handleIsReady() {
    isReady();
    std::cout << "readyok" << std::endl;
}

void UCIBridge::handleSetOption(std::istringstream& iss) {
...
void UCIBridge::handleNewGame() {
    newGame();
}

void UCIBridge::handlePosition(std::istringstream& iss) {
...
void UCIBridge::handleGo(std::istringstream& iss) {
    std::string token;
    int depth = -1;
    int movetime = -1;
    int wtime = -1;
...
void UCIBridge::handleStop() {
    stopSearch();
}

void UCIBridge::handleQuit() {
    quit();
}